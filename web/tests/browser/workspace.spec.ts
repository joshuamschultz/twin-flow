...
test('operator imports, maps, runs and exports a scenario', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto('http://127.0.0.1:5173');
  await expect(page.getByRole('heading', { name: /Understand today/ })).toBeVisible();
  await page.getByRole('button', { name: 'Spring Production flow & resource capacity' }).click();
  ...
  await page.screenshot({ path: '/private/tmp/twinflow-workspace-process.png', fullPage: true });
  ...
  await expect(page.getByRole('heading', { name: 'Order completion outcomes' })).toBeVisible({ timeout: 45000 });
  const [download] = await Promise.all([page.waitForEvent('download'), page.getByRole('button', { name: 'Evidence', exact: true }).click()]);
  ...
  await page.getByLabel('What flows, and in what order?').fill('Invoice approval then payment');
  ...
  await page.screenshot({ path: '/private/tmp/twinflow-workspace-library.png', fullPage: true });
  expect(errors).toEqual([]);
});

test('invalid import is actionable and mobile layout remains accessible', async ({ page }) => {
  ...
  await page.screenshot({ path: '/private/tmp/twinflow-workspace-mobile.png', fullPage: true });
