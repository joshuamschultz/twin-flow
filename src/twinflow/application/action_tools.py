...
class ActionTools:
    ...
    def approve(self, proposal_id: str, reviewed_digest: str, reviewer: str) -> dict[str, Any]:
        ...
        self.workspace.repository.append_audit(
            self.workspace.now(), "approve_dry_run", "proposals", proposal_id, "approved"
        )
        return {
    ...
    def deliver(
        self, proposal_id: str, approval_id: str, request_key: str, current_revision: str
    ) -> dict[str, Any]:
        record = self.workspace.repository.get("proposals", proposal_id)
        proposal = Proposal(**record["proposal"])
        entry = self.outbox.submit(proposal, approval_id, request_key, now=datetime.now(UTC))
        receipts = self.outbox.dispatch_pending(
            DryRunSink(),
            current_operational_revision=current_revision,
            entry_id=entry.entry_id,
            now=datetime.now(UTC),
        )
        return {
            "entry": asdict(self.outbox.get(entry.entry_id)),
            "receipts": [asdict(receipt) for receipt in receipts],
            "mode": "dry_run",
            "interpretation": (
                "No operational system was changed. Revision supplied for this dry run."
            ),
        }
